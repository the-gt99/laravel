...
<script>
        ...
        _addProduct(data) {
            $(`#productsList`).append(
                jQuery('<\li>', {
                    ...
                    'id': data.id,
                    'data-cost': data.cost,
                    'data-currency-name': data.currency.name,
    ...
    const SiteAPI = {
        ...
        async getProducts() {
            return await this._get(`${location.href}productList`)
        },
        async getCurrencies() {
            return await this._get(`${location.href}currenciesList`)
        },
        async getCurrencyByName(name) {
            return await this._get(`${location.href}currency/${name}`)
        },
    };
    ...
    const PageLogic = {
        ...
        _setListeners() {
            $(".product").click(async e => {
                ...
                let currencyName = el.dataset.currencyName;
                let currencyCost = el.dataset.currencyCost;

                let actualCurrency = await SiteAPI.getCurrencyByName(currencyName);
                ...
                let newCurrencyName = prompt('Введите название валюты:');
                let newCurrencyInf = await SiteAPI.getCurrencyByName(newCurrencyName)
